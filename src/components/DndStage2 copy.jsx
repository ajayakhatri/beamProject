...
// beamLength, setBeamLength, positionOnBeam, beamID, id, changeToolValue
export function DropableNew(props) {
    const [inputValue, setinputValue] = useState(props.positionOnBeam)
    const [actualPosition, setactualPosition] = useState(props.positionOnBeam)
    const [positionOnBeam, setpositionOnBeam] = useState(props.positionOnBeam)
    ...
    function handleMove(e) {
        ...
        setactualPosition(sliderRect.left - barRect.left)
        setpositionOnBeam((sliderRect.left - barRect.left + sliderRect.width / 2) * props.beamLength / beamLength())
        // console.log({
        //     "sliderRect.left": sliderRect.left,
        //     "barRect.left": barRect.left,
        //     "positionONBeam": (sliderRect.left - barRect.left + sliderRect.width / 2) * props.beamLength / beamLength()
        // })
        ...
        props.changeToolValue(props.beamID, props.id, "positionOnBeam", parseFloat(positionOnBeam))

        console.log(positionOnBeam)
        setinputValue(positionOnBeam)
    };
    const combinedStyle = {
        ...props.style,
            ...
            <div style={{ position: "absolute", bottom: "-20px", backgroundColor: "blue" }}>
                <input
                    ...
                    onBlur={(e) => {
                        ...
                        setactualPosition(parseFloat(newPosition) - getToolWidth() / 2)
                        setpositionOnBeam(parseFloat(newPosition))
                        setinputValue(actualPosition)
                    }} />
                hello
            </div>
        </div>
