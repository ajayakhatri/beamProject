...
loadStart: 5,
span:2,
img:<ImgDistributedLoad newSpanValue={2} scale={10 / actualBeamLength} spacing={20} loadEnd={5} loadStart={5} />
  }
]
      },
    }
  ...
  const [checkedLeft, setCheckedLeft] = useState(true);
  ...
  const setsetshowfig = (id,value) => {
    setshowFig((prevState) => ({
      ...prevState,
      [id]:value ,
    }));
  };
      ...
      {beams.map((beam) => (
          ...
          <ToggleButton
              ...
              onChange={(e) => {
                (
                  ...
                  beam.fixedSupportLeft?true:false||
                  beam.fixedSupportRight?true:false
                )? (setIsFigAvailable( e.currentTarget.checked),setshowFigNotAvailable(false),setsetshowfig(beam.id,true)):(setIsFigAvailable(false),
        ...
        {isFigAvailable&& (
           ...
           <SaveBeam beam={beam} changeOrAddBeamProperty={changeOrAddBeamProperty}/>
            ...
            <button className='btn btn-danger p-1' onClick={() => {console.log("showFig[beam.id]",showFig[beam.id],plot[beam.id],isFigAvailable)}}> Beam</button>
            <button className='btn btn-danger p-1' onClick={() => {deleteBeam(beam.id)}}>🗑️ Delete Beam</button>
          </div>
            {
              
        plot[beam.id] && isFigAvailable&& showFig[beam.id]&&
